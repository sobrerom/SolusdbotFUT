...
def backtest(prices: List[float], params: Dict[str, Any]) -> Dict[str, Any]:
    """Toy backtest that approximates grid PnL as price oscillation capture.
    ...
    for a, b in zip(prices, prices[1:]):
        # credit a small pnl when price crosses at least one grid step
        crosses = int(abs(b - a) // step)
        pnl += crosses * step * 0.001  # 0.1% capture per step crossed
        trades += crosses
