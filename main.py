...
class StreakBook:
    ...
    def record_entry(self, side, qty, entry_price, sl=None, tp=None, ts=None):
        o = {\"id\": int(time.time()*1000), \"side\": side, \"qty\": qty, \"entry\": entry_price,
             \"sl\": sl, \"tp\": tp, \"ts\": ts or int(time.time()*1000)}
        self.data.setdefault(\"open\", []).append(o); self.save(); return o[\"id\"]

    def mark_exit_if_crossed(self, mid):
        # naive: if price crosses tp/sl, close and score win/loss
        updated = False
        new_open = []
        ...
        if updated:
            ...
            # update streak stats
            wins = [c for c in closed[-5:] if c.get(\"result\")==\"win\"]
            last = closed[-1:] or []
            last_res = last[0].get(\"result\") if last else None
            streak = 0
            if last_res:
                # count consecutive same result from the end
...
def run():
    ...
    while not stopping and time.time() - start < cfg["daemon"]["max_runtime_seconds"]:
      ...
      if place_grid and trading_mode == "grid":
          ...
          last_mid = mid
          last_status = status
          # Loss reaction short-lived widening
          try:
              s = lad.data.get('stats',{})
              if s.get('last_result') == 'loss' and int(s.get('streak',0)) >= 1:
                  loss_cfg = cfg.get('loss_reaction',{})
                  widen_loops = int(loss_cfg.get('widen_loops',20))
                  k_tmp = cfg['grid']['k'] * float(loss_cfg.get('widen_k_mult',1.2))
                  _loss_until = ts + widen_loops * int(cfg.get('daemon',{}).get('loop_seconds',6))
                  # stash in state.json via extra field
                  pass
          except Exception:
              pass
